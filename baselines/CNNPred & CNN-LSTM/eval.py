...
if FLAGS.all_markets:
    ...
    dataset_test = StockMarketData(markets = [FLAGS.target_market], 
                                   train = False, split = 0.2, 
                                   target_market = FLAGS.target_market, 
                                   days = FLAGS.num_days)
    markets = 1
...
if FLAGS.regression:
    dataset_test = StockMarketDataReg(train = False, split = 0.2, 
                                      target_market = FLAGS.target_market, 
                                      days = FLAGS.num_days)
    var = dataset_test.var()

test_dataloader = Data.DataLoader(dataset_test, batch_size = FLAGS.batch_size, \
...
if FLAGS.regression:
    ...
    print(f"MSE on {FLAGS.target_market}, 
          RMSE: {rmse}, MAE:{mae}, 
          R-Squared:{r_squared}")
else:
    target_names = ["0", "1"]
    ...
    print(f"Average f-score for {FLAGS.target_market} input on All 
          Markets as {FLAGS.all_markets}: {accuracy}")
    
    print(f"Average f-score for {FLAGS.target_market} input on All 
          Markets as {FLAGS.all_markets}: {average_f_score}")
    
    print(f"Max f-score for {FLAGS.target_market} input on All 
          Markets as {FLAGS.all_markets}: {max_f_score}")