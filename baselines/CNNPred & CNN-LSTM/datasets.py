...
class StockMarketData(Data.Dataset):
    def __init__(self, markets : List[str], train : bool = True, \
                 split : float = 0.2, target_market : str = "NASDAQ", \
                    days : int = 60):
        self.days = days
    ...
    def __getitem__(self, idx):
        ...
        for a in self.markets:
            ...
            market_tens = T.reshape(T.from_numpy(
                np.array(df.iloc[idx: end_idx + 1])
            ), (self.features, self.days, 1))
