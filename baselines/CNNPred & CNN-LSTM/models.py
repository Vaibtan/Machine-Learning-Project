...
class CNNPred(nn.Module):
    def __init__(self, features, days, markets):
        ...
        #@model_desc: if markets = 1, model -> CNNPred2D else model -> CNNPred3D
    
        self.conv1 = nn.Conv2d(in_channels = features, out_channels = 8, \
                               kernel_size = (1, 1), stride = 1)
        
        self.conv2 = nn.Conv2d(in_channels = 8, out_channels = 8, \
                               kernel_size = (3, markets), stride = 1)
        ...
        self.fc = nn.Linear(in_features = 8 * int((int((days - 2) / 2) - 2) / 2), \
                            out_features = 2)
    ...
    def forward(self, x):
        #@input : (batch_size, features, days, markets)
        x = self.feat_act(self.conv1(x))
        x = self.feat_act(self.maxpool1(self.conv2(x)))
        x = self.feat_act(self.maxpool2(self.conv3(x)))
        x = self.flatten(x)
...
class CNN_LSTM(nn.Module):
    ...
    def forward(self, x):
        #@input : (batch_size, features, days)
        x = self.relu(self.tanh(self.cnn(x)))
        x = T.transpose(x, 1, 2)
        b = int(x.size()[0])
        ho = T.zeros(1 ,b, self.hidden_size).cuda()
        co = T.zeros(1 ,b, self.hidden_size).cuda()
