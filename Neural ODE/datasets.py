...
class StockMarketData(Data.Dataset):
    def __init__(self, markets, train = True, split = 0.2, \
                 target_market = "NASDAQ", days = 60):
        self.days = days
    ...
    def __getitem__(self, idx):
        ...
        for a in self.markets:
            ...
            market_tens = T.reshape(T.from_numpy(np.array(df.iloc[idx:end_idx+1])), (self.features, self.days, 1))
            final_tensor.append(market_tens)
        return T.cat(final_tensor, dim = 2).float(), lab
...
class StockMarketDataReg(Data.Dataset):
    def __init__(self, train = True, split = 0.2, target_market = "NASDAQ", days = 60):
        ...
        self.mean = u.mean()
        self.std = u.std()
        if not train: u = df.iloc[int(num_rows_init*(1-split)) - self.days:]
        self.num_rows = u.shape[0]
        u = (u - self.mean)/self.std
        self.market_data = u
    
    def get_label(self, idx):
        return T.tensor([float(self.market_data.iloc[idx + 1]["Close"])])
    
    def __getitem__(self, idx):
        end_idx = self.days + idx - 1
        lab = self.get_label(end_idx)
        df = self.market_data
        market_tens = T.reshape(T.from_numpy(
            np.array(df.iloc[idx:end_idx+1])), (self.features, self.days)
        )
...
class StockMarketDataTimeSeries(Data.Dataset):
    ...
    def get_label(self, idx):
        return T.tensor([float(self.market_data.iloc[idx]["Close"])])
    
    def __getitem__(self, idx):
        end_idx = self.days + idx - 1
        lab = self.get_label(end_idx)
        df = self.market_data
        close_tens = T.reshape(T.from_numpy(
            np.array(df.iloc[idx:end_idx+1]["Close"])), (self.days, 1)
        )
        time_stamp = T.reshape(T.from_numpy(
            np.linspace(0, self.days * 0.2 ,num = self.days)), (self.days, 1)
        )
...
if __name__ == "__main__":
    dataset = StockMarketData(train = True, split = 0.2, \
                              target_market = "NASDAQ", days = 60)
    u = dataset.__len__() - 1
