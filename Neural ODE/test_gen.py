...
vae.load_state_dict(T.load(f"{FLAGS.target_market}_generative.pt"))

dataset_test = StockMarketDataTimeSeries(train = False, split = 0.2, \
                                         target_market = FLAGS.target_market, \
                                            days = FLAGS.num_days)

dataloader_test = Data.DataLoader(dataset_test, batch_size = 1, shuffle = True, \
                                  num_workers = 2)
